...
Então("devo ver a notificação {string}") do |expect_alert|
     expect(@movie_page.form.alert).to have_content expect_alert
end
...
Então("este item deve ser removido do catálogo") do
    expect(@movie_page.has_no_movie(@movie['tltie'])).to be true
end
...
Então("este item deve permanecer no catálogo") do
    expect(@movie_page.has_movie(@movie['tltie'])).to be true
end